...
import uuid
import cv2
from config import SNAPSHOT_DIR, VIDEO_REENTRY_SECONDS
from db.jobs_repo import add_video_detection, update_video_appearance
...
class VideoEventRecorder:
    ...
    def record(self, detections, frame, timestamp_sec):
        ...
        for pid, det in confirmed.items():
            ...
            if previous and timestamp_sec - previous['last_seen'] <= VIDEO_REENTRY_SECONDS:
            ...
            try:
                ...
                event_id = add_video_detection(
                    ...
                    snapshot_path=f'static/snapshots/{face_name}', scene_path=f'static/snapshots/{scene_name}',
